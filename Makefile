...
OPENOCD_OPTIONS = -f /usr/share/openocd/scripts/interface/stlink-v2.cfg \
		  -f /usr/share/openocd/scripts/target/stm32f1x.cfg

# XXX Map/crossref output?
LD_SCRIPT	 = $(SRC_DIR)/ld/stm32_flash.ld
LDFLAGS		 = -lm \
		   $(ARCH_FLAGS) \
		   -static \
		   -Wl,-gc-sections \
...
# Flash
flash: $(TARGET_HEX)
	support/stmloader/stmloader -f $(TARGET_HEX)

flashn: $(TARGET_HEX)
	support/stmloader/stmloader -f $(TARGET_HEX) -n

stlink: $(TARGET_HEX)
	openocd $(OPENOCD_OPTIONS) -c "program $(TARGET_HEX) verify reset exit"

# Remake everything to make sure debug info is present, flash and launch gdb
gdb:
	make clean
	make DEBUG=GDB stlink
	arm-none-eabi-gdb  $(TARGET_ELF) -x support/openocd/stlink.scr
